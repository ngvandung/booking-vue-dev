        ...
        <v-col cols="12" lg="6" class="d-flex align-center">
          <v-menu offset-y v-for="(el, index) in dropdownSearchs" :key="index">
            <template v-slot:activator="{ on, attrs }">
              <v-btn height="30px" style="margin-right: 20px" v-bind="attrs" v-on="on">
              ...
              <v-card class="mx-auto my-12" max-width="374">
                ...
                <v-card-text style="padding: 0px 16px;">
                  <v-row align="center" class="mx-0">
                    ...
                    <div class="grey--text ml-4" style="font-size: 13px;">4.5 ({{home._source.countVote}})</div>
                  </v-row>
                  <div
    ...
    vm.$axios
      ...
      .then(async (response) => {
        for (let home of response.data.hits.hits) {
          ...
          vm.$axios
            .post(`http://localhost:8080/booking/api/v1/_search`, queryVote)
            .then((response) => {
